...
export default defineTool({
  ...
  async execute(input) {
    ...
    recordToolEvent("gh_list_issues", "requested", { state: input.state, labels: input.labels ?? null, assignee: input.assignee ?? null, limit: input.limit, repo: input.repo ?? process.env.GITHUB_REPOSITORY ?? null });
    const sandbox = await getSandbox();
    recordToolEvent("gh_list_issues", "sandbox_acquired");
...
ASH_SANDBOX_NODE`);
    ...
    recordToolEvent("gh_list_issues", "sandbox_command_finished", { exitCode: result.exitCode, durationMs: Date.now() - startedAt });
    if (result.exitCode !== 0) {
      recordToolEvent("gh_list_issues", "sandbox_command_failed", { exitCode: result.exitCode });
    ...
    if (!line) {
      recordToolEvent("gh_list_issues", "missing_result_marker", { stdoutBytes: result.stdout.length, stderrBytes: result.stderr.length });
    ...
    recordToolEvent("gh_list_issues", "completed", { count: output.total_count, durationMs: Date.now() - startedAt });
    return output;
  },
});
