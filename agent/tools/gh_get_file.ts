...
export default defineTool({
  ...
  async execute(input) {
    ...
    recordToolEvent("gh_get_file", "requested", { path: input.path, ref: input.ref ?? null, repo: input.repo ?? process.env.GITHUB_REPOSITORY ?? null });
    const sandbox = await getSandbox();
    recordToolEvent("gh_get_file", "sandbox_acquired");
...
ASH_SANDBOX_NODE`);
    ...
    recordToolEvent("gh_get_file", "sandbox_command_finished", { exitCode: result.exitCode, durationMs: Date.now() - startedAt });
    if (result.exitCode !== 0) {
      recordToolEvent("gh_get_file", "sandbox_command_failed", { exitCode: result.exitCode });
    ...
    if (!line) {
      recordToolEvent("gh_get_file", "missing_result_marker", { stdoutBytes: result.stdout.length, stderrBytes: result.stderr.length });
    ...
    recordToolEvent("gh_get_file", "completed", { type: output.type, path: output.path, size: output.size ?? null, durationMs: Date.now() - startedAt });
    return output;
  },
});
