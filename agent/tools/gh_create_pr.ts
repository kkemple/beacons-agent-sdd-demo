...
export default defineTool({
  ...
  async execute(input) {
    ...
    recordToolEvent("gh_create_pr", "requested", { title: input.title, head: input.head, base: input.base, draft: input.draft, repo: input.repo ?? process.env.GITHUB_REPOSITORY ?? null });
    const sandbox = await getSandbox();
    recordToolEvent("gh_create_pr", "sandbox_acquired");
...
ASH_SANDBOX_NODE`);
    ...
    recordToolEvent("gh_create_pr", "sandbox_command_finished", { exitCode: result.exitCode, durationMs: Date.now() - startedAt });
    if (result.exitCode !== 0) {
      recordToolEvent("gh_create_pr", "sandbox_command_failed", { exitCode: result.exitCode });
    ...
    if (!line) {
      recordToolEvent("gh_create_pr", "missing_result_marker", { stdoutBytes: result.stdout.length, stderrBytes: result.stderr.length });
    ...
    recordToolEvent("gh_create_pr", "completed", { number: output.number, state: output.state, url: output.url, durationMs: Date.now() - startedAt });
    return output;
  },
});
